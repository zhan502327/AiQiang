...
- (void)_setupSubview
{
    ...
    _redBagButton.translatesAutoresizingMaskIntoConstraints = NO;
    _redBagButton.backgroundColor = [UIColor redColor];
    [_redBagButton setTitle:@"偷红包" forState:UIControlStateNormal];
    _redBagButton.layer.masksToBounds = YES;
    _redBagButton.layer.cornerRadius = 5;
    [_redBagButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    _redBagButton.titleLabel.font = [UIFont systemFontOfSize:15];
    [_redBagButton addTarget:self action:@selector(redBagButtonClicked) forControlEvents:UIControlEventTouchUpInside];
    [self.contentView addSubview:_redBagButton];
    

    
    [self _setupAvatarViewConstraints];
    [self _setupButtonConstraints];//设置button 的约束
    [self _setupTitleLabelConstraints];
}

- (void)protectViewClicked{
    if (_progressViewBlock) {
        _progressViewBlock();
    }
}

- (void)redBagButtonClicked{
    if (_stealRedBagBlock) {
        _stealRedBagBlock();
    }
}
...
- (void)_setupTitleLabelConstraints
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.titleLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.redBagButton attribute:NSLayoutAttributeRight multiplier:1.0 constant:-EaseUserCellPadding]];
...
- (void)_setupButtonConstraints{
    
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.redBagButton attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:EaseUserCellPadding * 2 - 2]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.redBagButton attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-EaseUserCellPadding * 2 + 2]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.redBagButton attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:55]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.redBagButton attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-EaseUserCellPadding-10]];
