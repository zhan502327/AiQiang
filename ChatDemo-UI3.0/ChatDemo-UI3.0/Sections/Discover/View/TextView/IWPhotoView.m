...
- (void)setPhoto:(DBPhotoModel *)photo{
    _photo = photo;
    ...
    if ([photo.pic hasPrefix:@"http"] || [photo.pic hasPrefix:@"https"]) {
        ...
        NSString *encodeURL = [photo.pic stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];

        
        [self sd_setImageWithURL:[NSURL URLWithString:encodeURL] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            NSLog(@"%@",imageURL);
        }];
        [self sd_setImageWithURL:[NSURL URLWithString:encodeURL] placeholderImage:[UIImage imageWithName:@"app_default_img"]];
    } else {
        NSLog(@"%@",photo.asset.thumbnail);
