...
- (void)setupGroup:(voidBlock)endblock withSetupAsset:(BOOL)doSetupAsset
{
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        NSString *str = NSLocalizedStringFromTable(@"Not Allowed", @"UzysAssetsPickerController",nil);
        [self.btnTitle setTitle:str forState:UIControlStateNormal];
...
- (void)reloadData
{
    ...
    if (self.noShowImage) {
        ...
        for (int i = 0; i < self.assets.count; i++) {
            if (i == 0 && self.isTakePicture) {
                [array addObject:self.assets[i]];
            }
        }
        self.assets = array;
    }
...
- (void)setTitle:(NSString *)title
{
    ...
    [self.btnTitle setImageEdgeInsets:UIEdgeInsetsMake(5, self.btnTitle.titleLabel.frame.origin.x +self.btnTitle.titleLabel.frame.size.width + self.btnTitle.imageView.bounds.size.width + 5, 0, 0)];
    [self.btnTitle setTitleEdgeInsets:UIEdgeInsetsMake(5, 0, 0, 15)];
