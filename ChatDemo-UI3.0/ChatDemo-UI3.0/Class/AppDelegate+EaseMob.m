...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
{
    ...
    BOOL registerTokenSuccess = [GeTuiSdk registerDeviceToken:token];
    if (registerTokenSuccess == YES) {
        NSLog(@"向个推服务器注册DeviceToken成功");
    }else{
        NSLog(@"向个推服务器注册DeviceToken失败");
    }
    //环信
...
- (void)loginStateChange:(NSNotification *)notification
{
    ...
    NSLog(@"loginSuccess----------%d",loginSuccess);
    if (loginSuccess) {//登陆成功加载主窗口控制器
        //加载申请通知的数据
        [[ApplyViewController shareController] loadDataSourceFromLocalDB];
        if (self.mainController == nil) {
            self.mainController = [[UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle mainBundle]] instantiateViewControllerWithIdentifier:@"MainViewController"];
        } else {
