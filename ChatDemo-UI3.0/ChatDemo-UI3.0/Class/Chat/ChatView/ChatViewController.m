...
- (void)viewDidLoad {
    ...
    self.ext = @{FromNickName:User_NickName,FromHeading:heading};
...
//    //================png================
//    NSMutableArray *emotionPNG = [NSMutableArray array];
//    
//    for (int i = 1; i<91; i++) {
//        NSString *str = [NSString stringWithFormat:@"ee_%d",i];
//        EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:[NSString stringWithFormat:@"[(%@)]",str] emotionId:@"[(ee_66)]" emotionThumbnail:str emotionOriginal:str emotionOriginalURL:@"" emotionType:EMEmotionPng];
//        [emotionPNG addObject:emotion];
//        [_emotionDic setObject:emotion forKey:[NSString stringWithFormat:@"em%d",(10000 + i)]];
//    }
//    
//    EaseEmotionManager *managerPng = [[EaseEmotionManager alloc] initWithType:EMEmotionPng emotionRow:3 emotionCol:7 emotions:emotionPNG tagImage:[UIImage imageNamed:@"ee_1"]];
//    //===================================
//
//    //================gif================
...
//    //===================================
//    return @[managerPng,managerDefault,managerGif,managerPng];
    
    
//    NSMutableArray *dbEmotions = [NSMutableArray array];
//    NSMutableArray *dbNameArray = [NSMutableArray array];
//    _emotionDic = [NSMutableDictionary dictionary];
//    
//    for (int i = 0; i<90; i++) {
//        NSString *str = [NSString stringWithFormat:@"ee_%d",i+1];
//        [dbNameArray addObject:str];
//        
//    }
//    for (int i = 0; i<dbNameArray.count; i++) {
//        
//        NSString *name = dbNameArray[i];
//        
//        EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:@"" emotionId:[NSString stringWithFormat:@"[(%@)]",name] emotionThumbnail:name emotionOriginal:name emotionOriginalURL:@"" emotionType:EMEmotionPng];
//        [dbEmotions addObject:emotion];
//        [_emotionDic setObject:emotion forKey:[NSString stringWithFormat:@"[(ee_%d)]",(i+1)]];
//    }
//    EaseEmotionManager *dbEmotinManger= [[EaseEmotionManager alloc] initWithType:EMEmotionPng emotionRow:3 emotionCol:5 emotions:dbEmotions tagImage:[UIImage imageNamed:@"ee_1"]];
//    
//    return @[dbEmotinManger];
...
{
    NSString *heading = [NSString stringWithFormat:@"%@%@",www,User_Heading];
    return @{MESSAGE_ATTR_EXPRESSION_ID:easeEmotion.emotionId, FromNickName:User_NickName, FromHeading : heading, MESSAGE_ATTR_IS_BIG_EXPRESSION:@(YES)};
}
...
- (void)backAction
{
    ...
    if (self.conversation.type == EMConversationTypeGroupChat) {
        if ([self.conversation.conversationId isEqualToString:OneGroupId] || [self.conversation.conversationId isEqualToString:TwoGroupId] || [self.conversation.conversationId isEqualToString:ThreeGroupId] || [self.conversation.conversationId isEqualToString:FourGroupId]) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
                EMError *error = nil;
                [[EMClient sharedClient].groupManager leaveGroup:weakSelf.self.conversation.conversationId error:&error];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf hideHud];
                    if (error) {
...
//        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:NSLocalizedString(@"sureToDelete", @"please make sure to delete") delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"Cancel") otherButtonTitles:NSLocalizedString(@"ok", @"OK"), nil];
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"确定删除该聊天记录" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
