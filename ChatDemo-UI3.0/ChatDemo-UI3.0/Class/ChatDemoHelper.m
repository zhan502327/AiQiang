...
{
    NSDictionary *dic = @{@"uid":User_ID, @"get_uid" : aUsername};
    [PersonalInfoTool userInfoWithSuccessBlockWithPram:dic successBlock:^(UserInfoModel *model) {
        ...
        NSString *nickname = model.remark.length > 0 ? model.remark : model.nickname;
        
        NSString *msgstr = [NSString stringWithFormat:@"%@同意了加好友申请", nickname];
